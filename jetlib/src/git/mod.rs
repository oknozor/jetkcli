use git2::{Repository, Object, ObjectType, DiffOptions};
...
impl GitRepo {
    ...
    pub fn commit(&self, message: String) -> Result<(), JetError> {
        ...
        if !repo_is_empty && repo_has_deltas {
            ...
            self.repo.commit(Some("HEAD"), &sig, &sig, &message, &tree, &[&tip])
                .map(|_| ()).map_err(JetError::from)
            ...
            statuses.iter().for_each(|status| {
                eprintln!("{} : {:?}", status.path().unwrap(), status.status()); // TODO map status to string
            });
